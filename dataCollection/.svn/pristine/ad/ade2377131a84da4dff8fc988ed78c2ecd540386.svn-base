 ...
 * ***************************************************************************
 * @Package: com.synpowertech.dataCollectionJar.initialization
 * @ClassName: JedisUtil4signal
 * @Description: jedi缓存工具类
 * @version
 *-----------------------------------------------------------------------------
 *     VERSION           TIME                BY           CHANGE/COMMENT
 *-----------------------------------------------------------------------------
 *      1.0    2018年4月23日上午11:38:05   SP0010             create
 *-----------------------------------------------------------------------------
 ...
 */
@Component
public class JedisUtil4plantStatus {
	
	private static final Logger logger = LoggerFactory.getLogger(JedisUtil4plantStatus.class);
	...
	public void setShardedJedisPool(ShardedJedisPool shardedJedisPool) {
		JedisUtil4plantStatus.shardedJedisPool = shardedJedisPool;
		logger.info("shardedJedisPool4plantStatus is set successfully！");
	}

	static JedisUtil4plantStatus jedisUtil;
	...
	public void initMethod() {
		jedisUtil = new JedisUtil4plantStatus();
		logger.info("JedisUtil4plantStatus inits succefully！");
	...
	private static  ShardedJedis getShardedJedis() {
		...
		try {
			...
			logger.error("[JedisUtil4plantStatus] get shardedJedis false:" + e);
		} 
		return shardedJedis;
	}
...
//			if (shardedJedis != null && shardedJedis.exists(key)) {
	...
	/**
	  * @Title:  delStringInMap 
	  * @Description:  TODO 删除key对应map中字段和值
	  * @param key
	  * @param field: void
	  * @lastEditor:  SP0010
	  * @lastEdit:  2018年4月23日下午2:24:15
	 */
	public static void delStringInMap(String key, String field) {
		ShardedJedis shardedJedis = getShardedJedis();
		try {
			if (shardedJedis != null && shardedJedis.exists(key)) {
