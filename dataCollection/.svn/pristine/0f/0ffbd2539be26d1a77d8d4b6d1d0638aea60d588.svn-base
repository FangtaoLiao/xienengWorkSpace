...
public class SunRiseSet {
	...
	*/ 
	public static String getSunrise(double glong, double glat, String time, int zone) {
		double sunrise;
		int year, month, date;
		year = Util4j.getIntValue(time.split("-")[0],2001);
		month = Util4j.getIntValue(time.split("-")[1],1);
		date = Util4j.getIntValue(time.split("-")[2],1);
	...
	/**
	  ...
	  * @return: String
	  * @lastEditor:  SP0010
	  * @lastEdit:  2018年1月3日下午3:22:58
	 */
	public static String getSunrise(double glong, double glat, Calendar cal, int zone) {
		double sunrise;
		int year, month, date;
								...
								sun_deviation(
										...
										ecliptic_longitude(
												...
												G_sun(t_century(
														...
														UTo)))))), UTo, glong,
				glat, year, month, date);
		return dayStr + " " + (int) (sunrise / 15 + zone) + ":"+ (int) (60 * (sunrise / 15 + zone - (int) (sunrise / 15 + zone)));
	}
	...
	*/ 
	...
	/**
	  ...
	  * @return: String
	  * @lastEditor:  SP0010
	  * @lastEdit:  2018年1月3日下午3:23:55
	 */
	public static String getSunset(double glong, double glat, Calendar cal, int zone) {
		double sunset;
		int year, month, date;
		year = cal.get(Calendar.YEAR);
		month = cal.get(Calendar.MONTH) + 1;
								...
								sun_deviation(
										...
										ecliptic_longitude(
												...
												G_sun(t_century(
														...
														UTo)))))), UTo, glong,
				glat, year, month, date);
		return  dayStr + " " + (int) (sunset / 15 + zone) + ":"+ (int) (60 * (sunset / 15 + zone - (int) (sunset / 15 + zone)));
	}
...
/*	public static void main(String[] args) throws ParseException{
		System.out.println(getSunrise(104.0644200000,30.5688900000,"2018-01-04",8));
		System.out.println(getSunset(104.0636293034,30.5687090052,"2018-01-04",8));
		System.out.println(getSunrise(116.5657567978,34.7202921520,Calendar.getInstance(),8));
		System.out.println(getSunset(116.5657567978,34.7202921520,Calendar.getInstance(),8));
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date date = dateFormat.parse("2018-1-3 8:1:33");
